...
then
    ...
    dhcpcd
    warn "Wait several seconds to make sure that connection is up [RETURN]"
fi
...
mess "Cleaning mess - remove orphans recursively"
        ...
        if ! [ "${gitlinks[$i]}" == "" ]; then
            ...
            for f in $(ls -A ${gitfolders[$i]}/ | grep -v .git); do
                if [ -d ${gitlinks[$i]}/$f ]; then
                    mess "Move $f folder from ${gitlinks[$i]} to ${gitfolders[$i]} becaus it exists :)"
...
for (( i = 0; i < ${#users[@]}; i++ )); do
    ...
    passwd ${users[$i]}
    mess "Set ${users[$i]} shell to ${shells[$i]}"
    chsh -s ${shells[$i]} ${users[$i]}
    if ! [ "${gitname[$i]}" == "" ]; then
        mess "Prepare user-executed script for ${users[$i]} user"
        echo '
        source ceal.sh

        mess "Configure git for ${users[$i]}"
        ...
        mv /home/${users[$i]}/.zprofile /home/${users[$i]}/.zprofilecopy 2>/dev/null
        mv /home/${users[$i]}/.bash_profile /home/${users[$i]}/.bash_profilecopy 2>/dev/null
        su - ${users[$i]} -c user.sh
        mv /home/${users[$i]}/.zprofilecopy /home/${users[$i]}/.zprofile 2>/dev/null
        mv /home/${users[$i]}/.bash_profilecopy /home/${users[$i]}/.bash_profile 2>/dev/null
        rm user.sh
    fi
done
